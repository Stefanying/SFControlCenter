...
{
    public partial class UserOprationList : UserControl
    {
        ...
        private string _configPath = AppDomain.CurrentDomain.BaseDirectory + "CustomData.xml";
        public UserOprationList()
        {
            InitializeComponent();
        ...
        private void AddUserDefinedCommand()
        {
            if(_userDefinedNameList!=null &&_userDefinedNameList.Count>0)
            {
                自定义动作ToolStripMenuItem.DropDownItems.Clear();

                for (int i = 0; i < _userDefinedNameList.Count; i++)
                {
                    string _contextName = _userDefinedNameList[i].Name;
        ...
        void MenuClicked(object sender, EventArgs e)
        {
            ...
            if (_userDefinedNameList != null && _userDefinedNameList.Count > 0)
            {
                for(int i=0;i<_userDefinedNameList.Count;i++)
                {
                    if (_userDefinedNameList[i].Name == sender.ToString())
                    {
                        foreach (UserOperation operation in _userDefinedNameList[i].Operations)
                        {
        ...
        public void RefreshOprations()
        {
            ...
            if(_opreations != null && _opreations.Count > 0)
            {
                ...
                if (!(_currentOperation != null && _opreations.Contains(_currentOperation)))
                {
